...
<div class="card overflow-auto  col-3 bg-light ticket-container">
    <div class="d-flex justify-content-between align-items-center">
        <div class="card-header bg-light text-warning border-0">
            درحال انجام
            <span class="badge badge-pill badge-warning">{{$inprogressDemands->count()}}</span>
        </div>
        <div class="">
    ...
    @if ($inprogressDemands->isNotEmpty())
    ...
    <div class="card-body p-0">
        ...
        <div class="card   list-group-item-action  border-0 mb-2  shadow">
            <div class="card-body px-2 py-2" data-toggle="modal"
                data-target="#inprogress-demand{{$inprogressDemand->id}}">
                <div class="mt-1 p-0 btn">
                    <p class="text-small text-bold mb-0 text-right">{{$inprogressDemand->id}} -
                        {{$inprogressDemand->subject}}</p>
                </div>
        ...
        <!-- Inprogress Demand  Modal -->
        <div class="modal fade" id="inprogress-demand{{$inprogressDemand->id}}" tabindex="-1"
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <div class="d-flex">
                                        <div>
                                            ...
                                            <label>{!! jdate($inprogressDemand->created_at)->format('Y-m-d')!!}</label>
                                        </div>
                                        <div class="mr-3">
                                            <label class="text-bold">تاریخ ارجاع :</label>
                                            <label>{!! jdate($inprogressDemand->updated_at)->format('Y-m-d')!!}</label>
                                        </div>
                                    </div>
                                </div>
                                <div>
