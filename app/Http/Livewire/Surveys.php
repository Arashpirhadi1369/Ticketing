...
class Surveys extends Component
{
    ...
    public function mount(Survey $entity)
    {
        $this->entity = $entity;
    }
    ...
    public function storeQuestions()
    {
        ...
        if (isset($this->effectivenessQuestion[0])) {
            foreach ($this->effectivenessQuestion as $effectivenessQuestion) {
                SurveyQuestion::where('id', $effectivenessQuestion->id)->update(['question' => $effectivenessQuestion->question]);
            }
        } else {
            foreach ($this->questions as $question) {
                SurveyQuestion::create([
                    'survey_id' => $this->entity->id,
                    'question' => $question,
                ]);
            }
        }
    ...
    public function storeAnswers()
    {
        ...
        if (isset($this->effectivenessQuestionAnswer[0])) {
            foreach ($this->effectivenessQuestionAnswer as $effectivenessQuestionAnswer) {
                SurveyQuestionAnswer::where('id', $effectivenessQuestionAnswer->id)->update(['answer' => $effectivenessQuestionAnswer->answer]);
            }
        } else {
    ...
    public function edit(Survey $entity)
    {
        $this->entity = $entity;
    ...
    {
        $effectivenesse = Survey::find($this->entity->id);
